...
class AtencionPrenatalController {
   ...
   */
    ...
    async getTratamientoProcedimiento (req, res){
        try {
            let id_detalle = req.query.id_detalle;
            await atencionPrenatalService.getTratamientosProcedimientos(id_detalle).then((val) => {
                response.sendData(res, val, "success");}).catch(err=>{
                response.sendBadRequest(res, err.message);
            })
    ...
    async postAtencionPrenatal(req, res) {
        ...
        let gestante = await gestanteService.getOneGestante(
            req.body.atencion_prenatal.id_gestante
        );
        let atencionPrenatal = req.body.atencion_prenatal;
        try {
            let detalleAtencionPrenatal =
        await atencionPrenatalService.getOneAtencionPrenatal(
            atencionPrenatal.id_gestante,
            atencionPrenatal.id_num
        );
            if (!detalleAtencionPrenatal) {
                /* se crea la hoja */
                var newHojaCreated = await hojaAtencionService.postHojaAtencion(
                    t,
                    newHojaAtencion
                );
                ...
                try {
                    let edad = utils.ageCalculator(
                        fnacimiento[1] + "/" + fnacimiento[0] + "/" + fnacimiento[2],
                        newAtencion.fecha_atencion
                    );
                ...
                req.body.atencion_prenatal.id_atencion =
          detalleAtencionPrenatal.id_atencion;
                req.body.hoja_atencion.id_hoja_atencion =
          detalleAtencionPrenatal.his_atencion.his_hoja_atencion.id_hoja_atencion;
                req.body.atencion.id_atencion = detalleAtencionPrenatal.id_atencion;
                req.body.atencion.id_hoja_atencion =
          detalleAtencionPrenatal.his_atencion.his_hoja_atencion.id_hoja_atencion;
    ...
    async putAtencionPrenatal(req, res) {
        ...
        try {
            ...
            console.log("soy el error put", error);
            t.rollback();
            response.sendBadRequest(res, error.message);
        }
    }
