...
function printBodyPDF(doc, atencion, diag, position, tachar) {
    switch (position) {
    case 1:
        paintedPosition(doc, atencion, diag, 0, tachar)
        break;
    case 2:
        paintedPosition(doc, atencion, diag, 50, tachar)
        break;
    case 3:
        paintedPosition(doc, atencion, diag, 99, tachar)
        break;
    case 4:
        paintedPosition(doc, atencion, diag, 148, tachar)
        break;
    case 5:
        paintedPosition(doc, atencion, diag, 198, tachar)
        break;
    case 6:
        paintedPosition(doc, atencion, diag, 248, tachar)
        break;
    case 7:
        paintedPosition(doc, atencion, diag, 296.5, tachar)
        break;
    case 8:
        paintedPosition(doc, atencion, diag, 346, tachar)
        break;
    case 9:
        paintedPosition(doc, atencion, diag, 395.7, tachar)
        break;
    case 10:
        paintedPosition(doc, atencion, diag, 445, tachar)
        break;
    case 11:
        paintedPosition(doc, atencion, diag, 495, tachar)
        break;
    case 12:
        paintedPosition(doc, atencion, diag, 545, tachar)
        break;
...
function paintedPosition(doc, lista, diag, position, tachar) {
    ...
    if (diag.length > 0) {
        ...
        if (diag[0].his_labs[0]) {
            if (diag[0].his_labs[0].descripcion.length > 2) {
                doc.fontSize(5).text(diag[0].his_labs[0].descripcion, 495.5, 185 + position)
            } else {
        ...
        if (diag[0].his_labs[1]) {
            ...
            if (diag[0].his_labs[1].descripcion.length > 2) {
                doc.fontSize(5).text(diag[0].his_labs[1].descripcion, 508, 185 + position)
            } else {
                doc.fontSize(6).text(diag[0].his_labs[1].descripcion, 508, 185 + position)
            }

            // doc.fontSize(6).text(diag[0].his_labs[1].descripcion, 509, 185 + position)
        }
        if (diag[0].his_labs[2]) {
            if (diag[0].his_labs[2].descripcion.length > 2) {
                doc.fontSize(5).text(diag[0].his_labs[2].descripcion, 520.8, 185 + position)
            } else {
                doc.fontSize(6).text(diag[0].his_labs[2].descripcion, 520.8, 185 + position)
            }
        }
        ...
        if (diag[1]) {
            ...
            if (diag[1].his_labs[0]) {
                ...
                if (diag[1].his_labs[0].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[1].his_labs[0].descripcion, 495.5, 198.5 + position)
                } else {
            ...
            if (diag[1].his_labs[1]) {
                ...
                if (diag[1].his_labs[1].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[1].his_labs[1].descripcion, 508, 198.5 + position)
                } else {
                    doc.fontSize(6).text(diag[1].his_labs[1].descripcion, 508, 198.5 + position)
                }


            }
            if (diag[1].his_labs[2]) {
                if (diag[1].his_labs[2].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[1].his_labs[2].descripcion, 520.8, 198.5 + position)
                } else {
                    doc.fontSize(6).text(diag[1].his_labs[2].descripcion, 520.8, 198.5 + position)
                }
            }
        ...
        if (diag[2]) {
            ...
            if (diag[2].his_labs[0]) {
                if (diag[2].his_labs[0].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[2].his_labs[0].descripcion, 495.5, 211 + position)
                } else {
                    doc.fontSize(6).text(diag[2].his_labs[0].descripcion, 495.5, 211 + position)
                }
            }
            if (diag[2].his_labs[1]) {
                if (diag[2].his_labs[1].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[2].his_labs[1].descripcion, 508.8, 211 + position)
                } else {
                    doc.fontSize(6).text(diag[2].his_labs[1].descripcion, 508.8, 211 + position)
                }
            }
            if (diag[2].his_labs[2]) {
                if (diag[2].his_labs[2].descripcion.length > 2) {
                    doc.fontSize(5).text(diag[2].his_labs[2].descripcion, 521, 211 + position)
                } else {
